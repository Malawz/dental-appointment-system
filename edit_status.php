...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
		...
		$q = "UPDATE appointement SET status='$status' WHERE id=$id LIMIT 1";
...
$q = "SELECT regdate,status FROM appointement WHERE id=$id";
$result = @mysqli_query ($dbcon, $q);
...
{   // Valid user ID, display the form.
	...
	// Create the form
	echo '<p><label class="label" for="status">Status:</label>
